...
object Logic extends ClausalLogicParser {
  ...
  def hausdrfDist1[T <: Term](x: T, y: T): Double = (x, y) match {
  ...
  def haussdorff(x: Term, y: Term, accum: Double, rest: List[Tuple2[Term, Term]]): Double = (x, y, rest) match {
  ...
  def haussdorffOuter(x: Term, y: Term): Double = (x, y) match {
    case (x: Literal, y: Literal) =>
      if (x.functor != y.functor | x.arity != y.arity) {
        1.0
      } else {
